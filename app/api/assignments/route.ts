...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    try {
      const tableCheck = await client.query("SELECT to_regclass('public.assignments') AS reg")
    ...
    return NextResponse.json({ error: "No se pudo obtener assignments" }, { status: 500 })
  }
}
